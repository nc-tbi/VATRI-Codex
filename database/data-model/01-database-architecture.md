...
| `status` | TEXT | NOT NULL | CHECK (status IN ('pending_registration','registered','deregistered','transferred_out')) | Current registration lifecycle state. |
...
| `state` | TEXT | NOT NULL | CHECK (state IN ('active','submitted','overdue','preliminary_triggered')) | Obligation lifecycle state. |
...
| `rubrik_b_goods` | NUMERIC(18,2) | NOT NULL | DEFAULT 0 | EU sale goods value (Rubrik B). |
...
Current delivery uses Docker init scripts. Authoritative DDL is in `database/schemas/`. The Docker init script path must be updated to source from `database/schemas/` to maintain single source of truth.
