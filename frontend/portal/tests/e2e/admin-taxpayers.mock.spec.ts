...
test("@mocked registration required fields block submit and optional fields are omitted when blank", async ({ page }) => {
  ...
  const address = (postedBody as Record<string, unknown>).address as Record<string, unknown>;
...
test("@mocked find-taxpayer supports registration-id direct lookup and taxpayer-id fallback", async ({ page }) => {
    ...
    if (taxpayerId === "TXP-FALLBACK-001") {
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          registrations: [{ registration_id: "REG-FALLBACK-001" }],
        }),
      });
      return;
    }
  ...
  await page.route("**/registrations/*", async (route) => {
    ...
    if (id === "REG-DIRECT-001" || id === "REG-FALLBACK-001") {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({
          registration_id: id,
          taxpayer_id: id === "REG-DIRECT-001" ? "TXP-DIRECT-001" : "TXP-FALLBACK-001",
          cvr_number: "12345678",
  ...
  await lookupInput.fill("REG-DIRECT-001");
